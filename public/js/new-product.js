...
$(".main-btn.btn-new").click(() => {
	Swal.fire({
		...
		confirmButtonText:
			'<a onclick="formSubmit()">Đấu giá sản phẩm <i class="fas fa-gavel"></i></a>',
...
function validateNewProduct(event) {
	...
	const product = {
		...
		thumb: $(
			"#frmNew > div:nth-child(3) > div:nth-child(2) > div > div > div.jFiler-input > div.jFiler-input-caption > span"
		).text()
	};
	...
	if (product.name == "" || product.name == null) {
		msg.name
			.html(
				'<i class="fas fa-exclamation-triangle"></i>&nbsp;Tên sản phẩm không được để trống'
			)
			.css("display", "block");
		msg.name.scrollView();
		return false;
	...
	if (!(product.cate && product.subcate)) {
		msg.cate
			.html(
				'<i class="fas fa-exclamation-triangle"></i>&nbsp;Hãy chọn danh mục cho sản phẩm của bạn'
			)
			.css("display", "block");
		msg.cate.scrollView();
		return false;
	...
	if (product.beginPrice == 0 || product.beginPrice == null) {
		msg.beginPrice
			.html(
				'<i class="fas fa-exclamation-triangle"></i>&nbsp;Hãy nhập giá khởi điểm cho sản phẩm của bạn'
			)
			.css("display", "block");
		msg.beginPrice.scrollView();
		return false;
	} else {
		if (product.beginPrice < 10000) {
			msg.beginPrice
				.html(
					'<i class="fas fa-exclamation-triangle"></i>&nbsp;Giá khởi điểm tối thiểu là 10,000 đồng'
				)
				.css("display", "block");
			msg.beginPrice.scrollView();
			return false;
	...
	if (product.stepPrice == 0 || product.stepPrice == null) {
		msg.stepPrice
			.html(
				'<i class="fas fa-exclamation-triangle"></i>&nbsp;Hãy nhập bước giá cho sản phẩm của bạn'
			)
			.css("display", "block");
		msg.stepPrice.scrollView();
		return false;
	} else {
		if (product.stepPrice < 5000) {
			msg.stepPrice
				.html(
					'<i class="fas fa-exclamation-triangle"></i>&nbsp;Bước giá tối thiểu là 5,000 đồng'
				)
				.css("display", "block");
			msg.stepPrice.scrollView();
			return false;
	...
	if (product.immePrice != 0 && product.immePrice != null) {
		if (product.immePrice <= product.beginPrice) {
			msg.immePrice
				.html(
					'<i class="fas fa-exclamation-triangle"></i>&nbsp;Giá mua ngay phải lớn hơn giá khởi điểm'
				)
				.css("display", "block");
	...
	if (product.mainImg == "") {
		msg.mainImg
			.html(
				'<i class="fas fa-exclamation-triangle"></i>&nbsp;Sản phẩm phải có một ảnh chính'
			)
			.css("display", "block");
	...
	if (
		product.thumb.includes("Upload") ||
		product.thumb.includes("1") ||
		product.thumb.includes("2")
	) {
		msg.thumb
			.html(
				'<i class="fas fa-exclamation-triangle"></i>&nbsp;Sản phẩm phải có ít nhất 3 ảnh phụ'
			)
			.css("display", "block");
