...
function checkBid() {
	...
	// let begin = numeral($("span#begin").text()).value();
	let msg = $("p.err-validate#err-bidPrice");
	let min = current + step;
	if (numeral(a.val()).value() < min) {
		msg
			.html(
				`<i class="fas fa-exclamation-triangle"></i>&nbsp;Giá tối thiểu là ${numeral(
					min
				).format(0, 0) + " đ"}`
			)
			.css("display", "block");
	...
	// else if ((numeral(a.val()).value() - begin) % step != 0) {
	// 	msg
	// 		.html(
	// 			'<i class="fas fa-exclamation-triangle"></i>&nbsp;Giá đưa ra phải là bội của bước giá: ' +
	// 				$("span#step").text()
	// 		)
	// 		.css("display", "block");
	// 	return false;
...
$("input#bidPrice").focusout(() => {
	...
	if ($("#immePrice").length > 0) {
		if (
			numeral($("input#bidPrice").val()).value() >
			numeral($("#immePrice").text()).value()
		) {
...
$(".note-btn").click(() => {
	...
	var d = new Date();
	var strDate = `${d.getDate()}/${d.getMonth() +
