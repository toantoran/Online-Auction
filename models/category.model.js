...
module.exports = {
    ...
    getSingleSubCate : (cateID, subcateID) => db.load(`select * from category_sub where cateID = ${cateID} and subcateID = ${subcateID} order by subcateID`),
    ...
    deleteSubCateByID: (cateID, subcateID) => db.load(`delete from category_sub where cateID = ${cateID} and subcateID=${subcateID}`),
    ...
    editSubCate:  (entity) =>{
        return db.load(`update category_sub set subcateName = "${entity.subcateName}" where cateID= ${entity.cateID} and subCateID = ${entity.subcateID}`);
    },
};